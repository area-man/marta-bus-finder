...
server <- function(input, output, session) {
	...
	map <- leaflet() %>%
		...
		fitBounds(-84.540089, 33.643822, -84.172734, 33.871885) # City of Atlanta bias
	...
	observeEvent(input$update_bus_locations_button, {
		...
		leafletProxy("map") %>%
			...
			# should we only fit bounds the first time the route is loaded so we don't disturb user's selected bounds?
			leafletProxy("map") %>%
